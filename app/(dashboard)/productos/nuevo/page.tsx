...
export default function NuevoProductoPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try { await createProducto(form); router.push('/productos') } catch (err: any) { setError(err.message || 'Error.'); setLoading(false) }
  }
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 flex flex-col gap-4">
        ...
        <div className="bg-white border border-[#E5E4E0] rounded-xl overflow-hidden shadow-sm">
          <div className="bg-[#F9F9F8] border-b border-[#F1F0EE] px-4 py-3"><span className="font-display text-[13.5px] font-bold">Precios</span></div>
          <div className="p-4 grid grid-cols-2 gap-3">
            <Field label="Precio de venta"><input className={inp} type="number" min={0} step={0.01} value={form.precio_venta} onChange={e => set('precio_venta', parseFloat(e.target.value) || 0)} /></Field>
