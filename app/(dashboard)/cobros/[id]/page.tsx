...
export default function VerCobroPage() {
  ...
  const [r, setR] = useState<ReciboCobro | null>(null)
  const [metodos, setMetodos] = useState<
    { monto: number; cuentas?: { nombre: string } | null }[]
  >([])
  ...
  useEffect(() => {
    if (!id) return
    Promise.all([getReciboCobro(id), getMetodosCobro(id), getFacturasCobro(id), getRetencionesCobro(id)])
      .then(([rc, m, f, ret]) => {
        setR(rc as ReciboCobro)
        setMetodos(m || [])
        setFacturas(f || [])
        setRetenciones(ret || [])
      })
  ...
  async function handleAnular() {
    if (!id) return
    if (!confirm('¿Anular este recibo? Se revertirán los saldos de facturas y cuentas.')) return
    await anularReciboCobro(id)
    router.push('/cobros')
  }
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      <Topbar
        breadcrumb={[{ label: 'Cobros', href: '/cobros' }, { label: r.codigo }]}
        actions={
          <div className="flex gap-2">
            <button
              type="button"
              onClick={() => router.push('/cobros')}
              className="flex items-center gap-1.5 text-[11.5px] font-semibold px-3 py-1.5 rounded-[7px] border border-[#E5E4E0] bg-transparent text-[#6B6762] hover:border-[#A8A49D] transition-colors"
            >
              <ArrowLeft size={12} strokeWidth={2.2} /> Volver
            </button>
            <button
              type="button"
              onClick={() => window.open(`/cobros/${id}/imprimir`, '_blank')}
              className="flex items-center gap-1.5 text-[11.5px] font-semibold px-3 py-1.5 rounded-[7px] border border-[#E5E4E0] bg-white text-[#18181B] hover:border-[#A8A49D] transition-colors"
            >
      ...
      <div className="flex-1 min-h-0 overflow-y-auto p-6 flex flex-col gap-4">
        ...
        <div className="bg-white border border-[#E5E4E0] rounded-xl overflow-hidden shadow-sm">
          <div className="bg-[#F9F9F8] border-b border-[#F1F0EE] px-4 py-3">
            <span className="font-display text-[13.5px] font-bold">Facturas saldadas</span>
          </div>
          <table className="w-full border-collapse">
            <thead>
              <tr className="border-b border-[#F1F0EE] bg-[#F9F9F8]">
                {['Factura', 'Total', 'Importe abonado'].map((h, i) => (
                  <th
                    key={i}
        ...
        <div className="bg-white border border-[#E5E4E0] rounded-xl overflow-hidden shadow-sm">
          <div className="bg-[#F9F9F8] border-b border-[#F1F0EE] px-4 py-3">
            <span className="font-display text-[13.5px] font-bold">Métodos de cobro</span>
          </div>
          <table className="w-full border-collapse">
            <thead>
              <tr className="border-b border-[#F1F0EE] bg-[#F9F9F8]">
            ...
            <tbody>
              {metodos.map((m, i) => (
                <tr key={i} className="border-b border-[#F1F0EE] last:border-0">
                  ...
                  <td className="px-4 py-2.5 font-mono text-[12px] font-bold text-[#18181B]">{formatMonto(m.monto)}</td>
                </tr>
              ))}
            </tbody>
          </table>
          <div className="border-t border-[#E5E4E0] p-4 flex flex-col items-end gap-1.5">
            {totalRetenciones > 0 && (
              <div className="flex gap-8 text-[12px] text-[#6B6762]">
        ...
        {retenciones.length > 0 && (
          <div className="bg-white border border-[#E5E4E0] rounded-xl overflow-hidden shadow-sm">
            <div className="bg-[#F9F9F8] border-b border-[#F1F0EE] px-4 py-3">
              <span className="font-display text-[13.5px] font-bold">Retenciones</span>
            </div>
            <table className="w-full border-collapse">
              <thead>
                <tr className="border-b border-[#F1F0EE] bg-[#F9F9F8]">
        ...
        {!anulado && (
          <div className="pt-2 border-t border-[#E5E4E0]">
            <button
              ...
              className="flex items-center gap-1.5 text-[12px] font-semibold px-3.5 py-2 rounded-[9px] bg-[#FEE8E8] text-[#7F1D1D] border border-[#FECACA] hover:bg-[#EE3232] hover:text-white transition-colors"
            >
              <Trash2 size={13} strokeWidth={2} /> Anular recibo
            </button>
          </div>
        )}
      </div>
