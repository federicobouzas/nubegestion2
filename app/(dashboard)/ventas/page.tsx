...
export default function VentasPage() {
  ...
  const load = useCallback(async () => {
    ...
    let q = supabase
      .from('facturas_venta')
      .select('*, clientes(nombre_razon_social, cuit, condicion_iva)', { count: 'exact' })
                ...
                {facturas.length === 0 ? (
                  ...
                  return (
                    <tr key={f.id} className="border-b border-[#F1F0EE] last:border-0 hover:bg-[#FEF0EA] transition-colors group">
                      ...
                      <td className="px-4 py-3">{tieneCAE ? <span className="inline-flex items-center gap-1 text-[10px] font-semibold bg-[#E8F7EF] text-[#1A5C38] px-2 py-0.5 rounded-full"><span className="w-1.5 h-1.5 rounded-full bg-[#4EBB7F]" />CAE</span> : <span className="text-[11px] text-[#A8A49D]">Sin CAE</span>}</td>
