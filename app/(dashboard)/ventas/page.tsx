          ...
          <div className="bg-white border border-[#E5E4E0] rounded-xl overflow-hidden shadow-sm">
            <table className="w-full border-collapse">
              <thead><tr className="border-b border-[#E5E4E0] bg-[#F9F9F8]">
                {['Código','Número','Tipo','Cliente','Fecha','Total','Saldo','Estado','CAE',''].map((h,i)=><th key={i} className="font-mono text-[9.5px] tracking-[0.12em] uppercase text-[#A8A49D] px-4 py-2.5 text-left font-medium">{h}</th>)}
              </tr></thead>
              <tbody><TableSkeleton cols={['code','short','badge','medium','date','amount','amount','badge','badge','actions']} /></tbody>
            </table>
          </div>
        ) : (
          <div className="bg-white border border-[#E5E4E0] rounded-xl overflow-hidden shadow-sm">
              ...
              <tbody>
                {(() => {
                  const displayData = filtroEstado
                    ? data.filter((f: any) => calcularEstado(f.saldo_pendiente, f.fecha_vencimiento) === filtroEstado)
                    : data
                  if (displayData.length === 0) return <tr><td colSpan={10} className="px-4 py-10 text-center text-[#A8A49D] text-sm">No hay facturas.</td></tr>
