...
import { getFacturaCompra, getItemsFacturaCompra, getPercepcionesFacturaCompra, anularFacturaCompra, calcularEstadoCompra, formatMonto } from '@/lib/compras'
import type { FacturaCompra, ItemFacturaCompra } from '@/types/compras'

export default function VerCompraPage() {
  const { id } = useParams<{ id: string }>()
  const router = useRouter()
  const [f, setF] = useState<FacturaCompra | null>(null)
  const [items, setItems] = useState<ItemFacturaCompra[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    Promise.all([getFacturaCompra(id), getItemsFacturaCompra(id), getPercepcionesFacturaCompra(id)])
      .then(([fc, it]) => { setF(fc); setItems(it || []) })
      .catch(console.error).finally(() => setLoading(false))
  }, [id])

  async function handleAnular() {
    if (!confirm('¿Anular esta factura? Se revertirá el stock sumado. Esta acción no se puede deshacer.')) return
    await anularFacturaCompra(id)
    router.push('/compras')
  }
  ...
  const estadoStyle = { pagada: 'bg-[#E8F7EF] text-[#1A5C38]', pendiente: 'bg-[#FEF8E1] text-[#7A5500]', vencida: 'bg-[#FEE8E8] text-[#7F1D1D]' }[estado]
  const estadoLabel = { pagada: 'Pagada', pendiente: 'Pendiente', vencida: 'Vencida' }[estado]
  const anulada = f.notas === '[ANULADA]'

  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      <Topbar
        breadcrumb={[{ label: 'Compras', href: '/compras' }, { label: f.codigo }]}
        actions={
          <button onClick={() => router.push('/compras')} className="flex items-center gap-1.5 text-[11.5px] font-semibold px-3 py-1.5 rounded-[7px] border border-[#E5E4E0] bg-transparent text-[#6B6762] hover:border-[#A8A49D] transition-colors">
      ...
      <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-4">
        <div className="bg-white border border-[#E5E4E0] rounded-xl overflow-hidden shadow-sm">
          <div className="bg-[#F9F9F8] border-b border-[#F1F0EE] px-4 py-3 flex items-center justify-between">
            ...
            <span className={`inline-flex items-center gap-1.5 text-[11px] font-semibold px-2 py-0.5 rounded-full ${anulada ? 'bg-[#FEE8E8] text-[#7F1D1D]' : estadoStyle}`}>{anulada ? 'Anulada' : estadoLabel}</span>
          </div>
          <div className="p-4 grid grid-cols-4 gap-3">
            {[
              ['Proveedor', f.proveedores?.nombre_razon_social || '—'],
              ['CUIT', f.proveedores?.cuit || '—'],
              ...
              ['Condición', f.condicion_compra],
              ['Saldo', formatMonto(f.saldo_pendiente)],
              ['Total', formatMonto(f.total)],
            ].map(([l,v]) => (
