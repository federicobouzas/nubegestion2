...
import { formatMonto } from '@/lib/compras'
...
const emptyItem = (): ItemFacturaCompra => ({ tipo_item: 'producto', item_id: '', descripcion: '', cantidad: 1, precio_unitario: 0, iva_porcentaje: 21, descuento_porcentaje: 0, subtotal: 0 })
const emptyPerc = () => ({ tipo: '', numero_comprobante: '', importe: 0 })
const tiposPercepcion = ['IIBB', 'Imp. Internos', 'Municipal', 'IVA Percepción', 'Sellos']

interface Props { onSubmit: (data: IFacturaCompraForm) => Promise<void> }

export default function FacturaCompraForm({ onSubmit }: Props) {
  ...
  const [form, setForm] = useState<IFacturaCompraForm>({
    proveedor_id: '', numero: '', tipo: 'A',
    fecha_emision: today, fecha_vencimiento: '', periodo_desde: '', periodo_hasta: '',
    condicion_compra: 'Contado', notas: '',
    items: [emptyItem()], percepciones: [],
  })

  useEffect(() => {
    getProveedores({ estado: 'activo' }).then(d => setProveedores(d || []))
    getProductos({ estado: 'activo' }).then(d => setProductos(d || []))
  }, [])
  ...
  function setField(k: keyof IFacturaCompraForm, v: any) {
    setForm(p => ({ ...p, [k]: v }))
  ...
  function selectProveedor(id: string) {
    ...
    setProveedorSeleccionado(p || null)
  ...
  function setItem(idx: number, k: keyof ItemFacturaCompra, v: any) {
    setForm(p => {
      const items = [...p.items]
  ...
  function setPerc(idx: number, k: string, v: any) {
  ...
  function getStockHint(item_id: string) {
    ...
    if (!prod) return null
    if (prod.stock_actual <= 0) return { text: 'Sin stock', color: 'text-[#2CBAF2]' }
    const qty = pluralize(prod.stock_actual, prod.unidad_medida || 'unidad')
    return { text: `Stock actual: ${qty}`, color: 'text-[#1A5C38]' }
  }
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4 p-6">
        ...
        <Section title="Datos de la factura">
          <div className="grid grid-cols-3 gap-3">
            <div className="col-span-2">
              ...
              {proveedorSeleccionado && (
                <div className="mt-1.5 flex gap-3 text-[11px] text-[#6B6762]">
                  <span>CUIT: <strong>{proveedorSeleccionado.cuit || '—'}</strong></span>
                  <span>IVA: <strong>{proveedorSeleccionado.condicion_iva}</strong></span>
                </div>
              )}
            </div>
            ...
            <FieldWrapper label="Número factura proveedor">
            ...
            <FieldWrapper label="Condición de compra">
              <select className={inputCls()} value={form.condicion_compra} onChange={e => setField('condicion_compra', e.target.value)}>
                {['Contado','Cuenta corriente','Transferencia','Cheque'].map(v => <option key={v} value={v}>{v}</option>)}
              </select>
            </FieldWrapper>
        ...
        <div className="flex gap-2 justify-end">
          ...
          <button type="submit" disabled={loading} className="flex items-center gap-1.5 text-[12.5px] font-semibold px-4 py-2 rounded-[9px] bg-[#F2682E] text-white shadow-[0_3px_12px_rgba(242,104,46,0.30)] hover:bg-[#C94E18] transition-colors disabled:opacity-50"><Save size={13} strokeWidth={2.2} /> {loading ? 'Guardando...' : 'Guardar compra'}</button>
        </div>
      </form>
    </>
  )
