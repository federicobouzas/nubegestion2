...
import { formatMonto } from '@/lib/adelantos-clientes'
...
interface Props {
  initialData?: Partial<AdelantoClienteForm>
  onSubmit: (data: AdelantoClienteForm) => Promise<void>
  submitLabel?: string
  readonly?: boolean
...
export default function AdelantoClienteFormComp({
  initialData,
  onSubmit,
  submitLabel = 'Guardar',
  readonly = false,
  ...
  const [form, setForm] = useState<AdelantoClienteForm>({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.cliente_id) errs.cliente_id = 'El cliente es obligatorio.'
    ...
    <>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4 p-6">
          ...
          <div className="p-4 grid grid-cols-3 gap-3">
            <FieldWrapper label="Cliente" required error={errors.cliente_id}>
              {readonly ? (
                <input
                  className={inputCls()}
                  value={clientes.find(c => c.id === form.cliente_id)?.nombre_razon_social ?? form.cliente_id}
                  readOnly
                />
              ) : (
                <select
        ...
        <div className="flex gap-2 justify-end">
          <button
            type="button"
            onClick={() => router.push('/adelantos-clientes')}
